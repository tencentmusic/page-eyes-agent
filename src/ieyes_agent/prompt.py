...
2. 获取当前设备屏幕的元素信息（该操作为步骤的前置动作，禁止作为指令中单独的一个步骤输出）
3. 根据用户指令和当前屏幕的元素信息定位目标元素，如元素目标未出现，则判断是否存在"同意"、"确定"、"允许"、"跳过"、"关闭"、"取消"、"我知道了"、"Dismiss"、"X"，有则点击关闭后重新获取元素信息
4. 调用相应的工具执行操作
5. 如用户指令未完成，则重复操作1-4
6. 过程中如果出现位置、权限、用户协议、广告等弹窗，点击"同意"、"确定"、"允许"、"跳过"、"关闭"、"取消"、"我知道了"、"Dismiss"、"X"按钮关闭它。
