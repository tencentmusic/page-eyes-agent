...
description: >
  GUI 弹窗与遮罩层处理技能。当 GUI 自动化操作中遇到以下任何情况时，必须调用此技能：
  (1) 页面出现弹窗、遮罩层遮挡目标操作区域；
  (2) click/scroll/input 等操作因弹窗失败或返回错误；
  (3) 权限授权对话框（摄像头、麦克风、定位、通知）；
  (4) Cookie/隐私政策同意提示；
  (5) 广告弹窗/推广浮层；
  (6) 应用引导浮层/新手教程；
  (7) 登录/注册拦截弹窗；
  (8) 确认/提示对话框。
  处理原则：先调用本技能消除弹窗，再重试原始操作。禁止跳过本技能直接重试底层工具。
---

# GUI 弹窗处理技能

> **⚠️ 强制规则**：凡 GUI 操作受阻、工具返回失败、或屏幕存在任何弹窗/遮罩，**禁止跳过本技能直接重试底层工具**。必须先调用本技能处理弹窗，再继续操作。

## ⚠️ 必须调用场景

以下场景**必须**调用本 skill，禁止跳过直接操作：

- **页面出现弹窗遮挡操作区域**：任何弹窗、浮层、遮罩层覆盖了目标操作元素，导致无法点击或交互
- **`click` / `scroll` 等操作因弹窗失败**：工具返回失败且屏幕存在弹窗或遮罩层时
- **权限授权弹窗出现**：系统弹出摄像头、麦克风、定位、通知等权限请求对话框
- **Cookie / 隐私政策同意提示出现**：页面底部或居中出现 Cookie 同意横幅或弹窗
- **广告弹窗 / 推广浮层出现**：应用内广告、活动推广、会员推荐等弹窗遮挡页面
- **应用引导浮层 / 新手教程出现**：首次使用或新功能引导浮层覆盖页面
- **登录 / 注册拦截弹窗出现**：未登录操作触发登录弹窗，阻断当前任务流程
- **确认 / 提示对话框出现**：操作确认框、警告框、信息提示框等需要响应后才能继续

> 💡 **处理原则**：先调用本 skill 消除弹窗，再重试原始操作。

## 概述

在 GUI 自动化操作中，弹窗和遮罩层是最常见的操作中断因素。此技能提供一套系统化的识别与处理策略，指导智能体快速消除各类弹窗遮罩，恢复正常操作流程。
...
若弹窗虽未完全覆盖页面但遮挡了关键操作区域：

1. 尝试拖动弹窗到其他位置（如弹窗支持拖动）
2. 尝试调整页面滚动位置
3. 优先关闭弹窗后再继续操作
